"""
Antes este módulo armaba el PDF del RIDE acá mismo (reportlab + barcode);
ahora ese armado vive en sri_facturacion_service (ver
sri_facturacion_service/comprobantes/ride.py), que es quien tiene los datos
completos del comprobante. Acá solo queda pedirle el PDF ya armado — mismo
nombre de función y misma firma (`build_ride_pdf(comprobante)`) que antes,
para que billing/views.py y facturacion_electronica/views.py sigan
...
def build_ride_pdf(comprobante):
    ...
    try:
        response = requests.get(
            _url(f'/api/comprobantes/{comprobante.clave_acceso}/ride/'), headers=_headers(),
