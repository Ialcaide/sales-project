document.addEventListener('DOMContentLoaded', function () {
    ...
    // 3. Agregar botón "Consultar cuánto voy a pagar" al lado del botón de Registrar
    const submitRow = document.querySelector('.submit-row');
    if (submitRow) {
        const btnConsultar = document.createElement('input');
        btnConsultar.type = 'button';
        btnConsultar.value = 'Consultar cuánto voy a pagar';
        btnConsultar.style.backgroundColor = '#2563eb';
        btnConsultar.style.color = '#fff';
        btnConsultar.style.fontWeight = 'bold';
        btnConsultar.style.marginRight = '10px';
        btnConsultar.style.padding = '10px 15px';
        btnConsultar.style.border = 'none';
        btnConsultar.style.borderRadius = '4px';
        btnConsultar.style.cursor = 'pointer';
        
        // Insertar al inicio de la submit-row para que quede al lado de los botones de Guardar
        submitRow.insertBefore(btnConsultar, submitRow.firstChild);
        
        btnConsultar.addEventListener('click', function(e) {
            recalculate();
            const box = document.getElementById('prestamo-simulator-box');
            if (box) {
