...
(function () {
  document.addEventListener('DOMContentLoaded', function () {
    ...
    /* Estas 3 declaraciones tienen que vivir ACÁ (antes de toggleFormaPago,
       que las usa) y no más abajo junto al resto de la pasarela: más abajo
       en este archivo hay una llamada síncrona a toggleConsumidorFinal()
       (ver "fijarModoClienteInicial") que dispara toggleFormaPago() durante
       la configuración inicial de la página — si guardarFacturaBtn todavía
       no existiera en ese momento, el script se rompe con un TypeError ahí
       mismo y ningún listener definido después (el del botón "Pagar"
       incluido) llega a registrarse. */
    var guardarFacturaBtn = document.getElementById('guardar-factura-btn');
    ...
    function actualizarEstadoPagoTarjeta() {
      if (formaPagoSelect.value !== 'tarjeta') {
        guardarFacturaBtn.disabled = false;
        return;
      }
      guardarFacturaBtn.disabled = !tarjetaConfirmada;
    ...
    /* PayPal no comparte nada del flujo de tarjeta/efectivo: no hay pasarela
       simulada ni "Guardar Factura" a deshabilitar — el propio submit real
       ya crea la orden en PayPal y redirige al checkout real (ver
       billing/views.py). Acá solo se muestra el panel + botón dedicado
       (el botón vive DENTRO de paypal-wrapper, así que ocultar el wrapper
       ya lo oculta a él también) en vez del botón genérico. */
    var paypalWrapper = document.getElementById('paypal-wrapper');
    var paypalPagarBtn = document.getElementById('paypal-pagar-btn');
    var paypalPanelTotal = document.getElementById('paypal-panel-total');

    function togglePaypalCampos() {
      var esPaypal = formaPagoSelect.value === 'paypal';
      paypalWrapper.classList.toggle('d-none-force', !esPaypal);
      guardarFacturaBtn.classList.toggle('d-none-force', esPaypal);
    }
    formaPagoSelect.addEventListener('change', togglePaypalCampos);
    ...
    var cajeroDisplay = document.querySelector('.cajero-display');
    var cajeroRecibidoDisplay = document.getElementById('cajero-recibido-display');
    var cajeroCambioLabel = document.getElementById('cajero-cambio-label');
    var cajeroCambioDisplay = document.getElementById('cajero-cambio-display');
    ...
    function actualizarFeedbackModal() {
      ...
      cajeroRecibidoDisplay.textContent = '$' + monto.toFixed(2);
      if (monto <= 0) {
        modalFeedback.textContent = '';
        modalFeedback.className = 'mt-2 small';
        cajeroDisplay.classList.remove('is-insuficiente');
        cajeroCambioLabel.textContent = 'Cambio a devolver';
        cajeroCambioDisplay.textContent = '$0.00';
      } else if (monto < total) {
        ...
        cajeroDisplay.classList.add('is-insuficiente');
        cajeroCambioLabel.textContent = 'Falta';
        cajeroCambioDisplay.textContent = '$' + (total - monto).toFixed(2);
      } else {
        modalFeedback.textContent = 'Cambio a devolver: $' + (monto - total).toFixed(2);
        modalFeedback.className = 'mt-2 small text-success fw-bold';
        cajeroDisplay.classList.remove('is-insuficiente');
        cajeroCambioLabel.textContent = 'Cambio a devolver';
        cajeroCambioDisplay.textContent = '$' + (monto - total).toFixed(2);
      }
    }
    modalMontoInput.addEventListener('input', actualizarFeedbackModal);

    /* Botones de billetes: van SUMANDO al monto ya escrito (como un cajero
       real que va apilando lo que recibe), no lo reemplazan. */
    document.querySelectorAll('.cajero-billete').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var actual = parseFloat(modalMontoInput.value) || 0;
        var denominacion = parseFloat(btn.dataset.monto) || 0;
        modalMontoInput.value = (actual + denominacion).toFixed(2);
        actualizarFeedbackModal();
      });
    });

    document.getElementById('cajero-exacto').addEventListener('click', function () {
      modalMontoInput.value = getTotalActual().toFixed(2);
      actualizarFeedbackModal();
    });

    document.getElementById('cajero-limpiar').addEventListener('click', function () {
      modalMontoInput.value = '';
      actualizarFeedbackModal();
    });
    ...
    /* ---------- Pasarela de pago simulada (pago con tarjeta) ----------
       A diferencia de efectivo (que se resuelve recién al tocar "Guardar
       Factura"), tarjeta exige un paso previo explícito: hay un botón
       "Pagar" propio en el paso 3, y "Guardar Factura" queda DESHABILITADO
       mientras no se complete ese pago simulado — no alcanza con llenar
       los campos, hay que tocar "Pagar" y que la pasarela lo apruebe.
       100% decorativo, no hay ninguna pasarela real integrada (Stripe, etc.). */
    ...
    /* ---------- PayPal real: solo feedback visual, no intercepta el submit ----------
       "Pagar con PayPal" es un <button type="submit"> normal: el <form> se
       envía tal cual, el servidor crea la orden real en PayPal y redirige a
       paypal.com (ver billing/views.py). Este modal no frena nada — solo
       cubre visualmente el ratito que tarda esa llamada real antes de la
       redirección. Si el servidor la rechaza (PayPalError) y re-renderiza
       esta misma página con un error, la recarga completa del navegador ya
       se encarga de que el modal desaparezca solo. */
    var paypalRedirigiendoModal = new bootstrap.Modal(document.getElementById('paypalRedirigiendoModal'));
    paypalPagarBtn.addEventListener('click', function () {
      paypalRedirigiendoModal.show();
    });
    ...
    invoiceForm.addEventListener('submit', function (e) {
      if (formaPagoSelect.value === 'efectivo' && !montoConfirmado) {
        ...
        // en "Guardar Factura" (ej. Enter dentro de un campo de texto,
        // que no respeta el atributo disabled del botón), igual se corta
        // acá y se obliga a pasar por la pasarela.
        e.preventDefault();
        abrirPasarelaModal();
      }
    });
    ...
    function calcTotals() {
      ...
      if (paypalPanelTotal) paypalPanelTotal.textContent = '$' + total.toFixed(2);
    }
